    ...
    for addr in address:
        ...
        if not start_time or not end_time:
            ...
            customer = Customer.objects.filter(
                address=addr,
                outorder__outorderclothes__create_time__range=[start_time, end_time],
            ).values(
                "id",
                "address",
                "outorder__outorderclothes__amount",
...
if __name__ == "__main__":
    foo()
address = InorderClothes.objects.values_list("id", flat=True)

        # 初始化一个空列表，用于存储客户信息
        customers = []
        # 遍历每个地址
        for inorderClothes_id in address:
            customer = InorderClothes.objects.filter(id=inorderClothes_id).values(
                'id',
        ...
        for customer in customers:
            for cust in customer:
                d = {}
            # 如果当前客户的衣物名称不为空
            if cust["inorder__clothes__name"] is not None:
                # 将当前客户的信息添加到字典中
                d["id"] = cust["id"]
                d["inorder__clothes__name"] = cust["inorder__clothes__name"]
                d["inorder__clothes__sn"] = cust["inorder__clothes__sn"]
                d["amount"] = cust["inorder__clothes__price"]
                d["price"] = cust["inorder__inorderclothes__amount"]
                # 将字典添加到列表中
                if d not in customer_list:
                    customer_list.append(d)

                print("末尾：",customer_list)
        amount_sum = 0
        amount_sum_dict = {}
        total_price = 0
        for item in customer_list:
            amount_sum = sum(item["price"] for item in customer_list)
            price = item["price"]
            amount = item["amount"]
