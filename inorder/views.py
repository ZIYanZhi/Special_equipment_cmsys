...
from common.models import Inorder, User, InorderClothes, Customer, Clothes
...
def delete(request, inorder_id):
    qs = InorderClothes.objects.filter(inorder_id=inorder_id)
    if qs:
        messages.add_message(request, messages.WARNING, '删除失败，请先删除该单下的器材')
    ...
    if request.method == "POST":
        ...
        if inorderclothes_form.is_valid():
            ...
            with transaction.atomic():  # 事务
                new_inorderclothes = InorderClothes.objects.create(clothes=clothes, inorder_id=inorder_id, amount=amount)
                # 入库增加库存
